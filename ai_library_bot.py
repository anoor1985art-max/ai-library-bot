...
FACESWAP_TEMPLATES = [
    {"id": "t1", "name": "🕺 رقصة مضحكة كوميدية", "url": "https://raw.githubusercontent.com/intel-iot-devkit/sample-videos/master/person-bicycle-car-detection.mp4"},
    {"id": "t2", "name": "🚀 رائد فضاء يسبح في الفضاء", "url": "https://www.w3schools.com/html/movie.mp4"},
    {"id": "t3", "name": "🎭 البطل الخارق الطائر", "url": "https://www.w3schools.com/html/mov_bbb.mp4"}
]

SINGER_VOICES = [
    {"id": "abdelhalim", "name": "🎤 عبد الحليم حافظ"},
    {"id": "umkulthum", "name": "🎤 كوكب الشرق أم كلثوم"},
    {"id": "jackson", "name": "🎤 مايكل جاكسون"}
]
...
def call_replicate_model(version, input_data):
    if not REPLICATE_API_TOKEN:
        return False, "مفتاح API الخاص بـ Replicate غير متوفر في الإعدادات."
    ...
    try:
        r = requests.post("https://api.replicate.com/v1/predictions", json=payload, headers=headers, timeout=20)
        if r.status_code != 201:
            return False, f"فشل الاتصال بـ Replicate: {r.text}"
            
        pred_data = r.json()
        poll_url = pred_data.get("urls", {}).get("get")
        if not poll_url:
            return False, "لم يتم العثور على رابط الاستعلام للمهمة."
            
        while True:
            poll_r = requests.get(poll_url, headers=headers, timeout=10)
            if poll_r.status_code != 200:
                return False, f"فشل الاستعلام عن الحالة: {poll_r.text}"
                
            status_data = poll_r.json()
            status = status_data.get("status")
            
            if status == "succeeded":
                return True, status_data.get("output")
            elif status in ["failed", "canceled"]:
                return False, f"فشلت معالجة الذكاء الاصطناعي: {status_data.get('error')}"
                
            time.sleep(2.5)
            
    except Exception as e:
        return False, f"حدث خطأ أثناء الاتصال بالخادم: {e}"
...
if bot:
    ...
    def handle_start(message):
        ...
        welcome_text = (
            ...
            f"أنا مساعدك الخارق متعدد المواهب، أجمع بين <b>موسوعة الكتب والمراجع العالمية</b>، واستوديو <b>دمج الوجوه وتغيير نبرة الأصوات</b>، وبين <b>استوديو توليد الصور والفيديو (FLUX & AI Studio المجاني)</b>، ومساعد المحادثة والترجمة الذكية.\n\n"
    ...
    def handle_all_callbacks(call):
        ...
        if data == "mode_faceswap":
            ...
            markup = types.InlineKeyboardMarkup(row_width=1)
            markup.add(
                types.InlineKeyboardButton("🎭 تبديل الوجوه في الفيديو (Face Swap)", callback_data="sw_select_faceswap"),
                types.InlineKeyboardButton("🎤 تغيير نبرة الصوت (Voice Cover)", callback_data="sw_select_voiceswap"),
                types.InlineKeyboardButton("🔙 العودة للقائمة الرئيسية", callback_data="back_to_start")
            )
            bot.edit_message_text(
                "🎭 <b>استوديو دمج الوجوه والأصوات بالذكاء الاصطناعي</b>\n\n"
                "مرحباً بك في الاستوديو الترفيهي! اختر الأداة المطلوبة للبدء:",
            ...
            bot.edit_message_text(welcome_text, chat_id=chat_id, message_id=msg_id, reply_markup=get_main_menu_markup())

        elif data == "sw_select_faceswap":
            markup = types.InlineKeyboardMarkup(row_width=1)
            for t in FACESWAP_TEMPLATES:
                markup.add(types.InlineKeyboardButton(t['name'], callback_data=f"sw_tpl_sel|{t['id']}"))
            markup.add(types.InlineKeyboardButton("🔙 العودة", callback_data="mode_faceswap"))
            bot.edit_message_text(
                "🎭 <b>تبديل الوجوه في الفيديو (Face Swap)</b>\n\n"
                "اختر أحد القوالب الترفيهية والمضحكة أدناه لدمج وجهك عليه:",
                chat_id=chat_id, message_id=msg_id, reply_markup=markup
            )

        elif data.startswith("sw_tpl_sel|"):
            t_id = data.split("|")[1]
            tpl = next((t for t in FACESWAP_TEMPLATES if t["id"] == t_id), None)
            if tpl:
                user_states[chat_id] = "waiting_face_photo"
                user_swap_data[chat_id] = {"selected_template_id": t_id}
                bot.edit_message_text(
                    f"📸 <b>قمت باختيار قالب: {tpl['name']}</b>\n\n"
                    f"يرجى الآن إرسال <b>صورة وجهك (صورة سيلفي واضحة وذات إضاءة جيدة)</b> كصورة مباشرة في هذه الدردشة لبدء عملية الدمج:",
                    chat_id=chat_id, message_id=msg_id,
                    reply_markup=types.InlineKeyboardMarkup().add(types.InlineKeyboardButton("❌ إلغاء وتراجع", callback_data="mode_faceswap"))
                )

        elif data == "sw_select_voiceswap":
            markup = types.InlineKeyboardMarkup(row_width=1)
            for s in SINGER_VOICES:
                markup.add(types.InlineKeyboardButton(s['name'], callback_data=f"sw_voice_sel|{s['id']}"))
            markup.add(types.InlineKeyboardButton("🔙 العودة", callback_data="mode_faceswap"))
            bot.edit_message_text(
                "🎤 <b>تحويل نبرة الصوت لغناء المشاهير (AI Voice Cover)</b>\n\n"
                "اختر نبرة صوت الفنان الذي ترغب في أن يغني مقطعك الصوتي:",
                chat_id=chat_id, message_id=msg_id, reply_markup=markup
            )

        elif data.startswith("sw_voice_sel|"):
            voice_id = data.split("|")[1]
            user_states[chat_id] = "waiting_voice_audio"
            user_swap_data[chat_id] = {"selected_template_id": voice_id}
            bot.edit_message_text(
                f"🎙️ <b>تم تحديد الفنان بنجاح!</b>\n\n"
                f"يرجى الآن إرسال <b>المقطع الصوتي (سواء تسجيل صوتك بالمايكرفون أو ملف أغنية MP3)</b> لكي يعاد غناؤها بصوت الفنان المختار:",
                chat_id=chat_id, message_id=msg_id,
                reply_markup=types.InlineKeyboardMarkup().add(types.InlineKeyboardButton("❌ إلغاء وتراجع", callback_data="mode_faceswap"))
            )

        if data.startswith("extended_search|"):
            book_id = data.split("|")[1]
            book = books_cache.get(book_id)
            if not book:
                bot.answer_callback_query(call.id, "❌ لم يتم العثور على بيانات الكتاب.", show_alert=True)
                return
            
            title = book['title']
            authors = book['authors']
            
            q_noor = urllib.parse.quote(f"{title} {authors}")
            q_archive = urllib.parse.quote(f"{title} {authors}")
            q_hindawi = urllib.parse.quote(f"{title}")
            q_libgen = urllib.parse.quote(f"{title} {authors}")
            q_scholar = urllib.parse.quote(f"{title} {authors}")
            q_google = urllib.parse.quote(f"تحميل كتاب {title} {authors} pdf مجانا")
            
            text = (
                f"🔍 <b>خيارات البحث الموسع عن كتاب:</b>\n"
                f"📕 <i>«{title}»</i> - {authors}\n"
                f"━━━━━━━━━━━━━━━━━━\n"
                f"اختر محرك البحث أو المكتبة التي ترغب بالبحث فيها عن النسخ والروابط المتاحة للتحميل والقراءة مجاناً: 👇"
            )
            
            markup = types.InlineKeyboardMarkup(row_width=1)
            markup.add(
                types.InlineKeyboardButton("📚 بحث في مكتبة نور (Noor-Book)", url=f"https://www.noor-book.com/en/search?q={q_noor}"),
                types.InlineKeyboardButton("🏛️ بحث في أرشيف الإنترنت (Internet Archive)", url=f"https://archive.org/search.php?query={q_archive}+AND+mediatype:texts"),
                types.InlineKeyboardButton("📖 بحث في مؤسسة هنداوي (Hindawi)", url=f"https://www.hindawi.org/books/search/?q={q_hindawi}"),
                types.InlineKeyboardButton("🔬 بحث في مكتبة جينيسيس العالمية (LibGen)", url=f"https://libgen.is/search.php?req={q_libgen}"),
                types.InlineKeyboardButton("🎓 بحث في جوجل للأبحاث الأكاديمية (Google Scholar)", url=f"https://scholar.google.com/scholar?q={q_scholar}"),
                types.InlineKeyboardButton("⚡ بحث جوجل المباشر عن ملف PDF", url=f"https://www.google.com/search?q={q_google}"),
                types.InlineKeyboardButton("🔙 العودة لتفاصيل الكتاب", callback_data=f"book_details|{book_id}")
            )
            
            if call.message.caption:
                bot.edit_message_caption(text, chat_id=chat_id, message_id=call.message.message_id, reply_markup=markup)
            else:
    ...
    def send_book_details_card(chat_id, book):
        ...
        markup.add(
            types.InlineKeyboardButton("🔍 خيارات البحث الموسع عن روابط تحميل PDF", callback_data=f"extended_search|{book['id']}"),
    ...
    def handle_incoming_media_swap(message):
        chat_id = message.chat.id
        ...
        if isinstance(state, dict) and state.get("state") == "waiting_pdf_images" and message.content_type == "photo":
            ...
            active_tasks[chat_id] = True
            status_msg = bot.send_message(chat_id, "⏳ <b>جاري تحميل صورة وجهك وتجهيزها للمعالجة...</b>", parse_mode="HTML")
            try:
                file_info = bot.get_file(message.photo[-1].file_id)
                downloaded = bot.download_file(file_info.file_path)
                
                local_path = os.path.join(MEDIA_DIR, f"face_{chat_id}_{uuid.uuid4().hex[:6]}.jpg")
                with open(local_path, "wb") as f:
                    f.write(downloaded)
                    
                bot.edit_message_text("🚀 <b>جاري رفع الصورة إلى الخوادم السحابية...</b>", chat_id=chat_id, message_id=status_msg.message_id)
                
                face_url = upload_to_tmpfiles(local_path)
                if not face_url:
                    bot.edit_message_text("❌ فشل رفع الصورة، يرجى المحاولة مرة أخرى.", chat_id=chat_id, message_id=status_msg.message_id)
                    return
                    
                swap_info = user_swap_data.get(chat_id, {})
                t_id = swap_info.get("selected_template_id")
                tpl = next((t for t in FACESWAP_TEMPLATES if t["id"] == t_id), None)
                if not tpl:
                    bot.edit_message_text("❌ لم يتم تحديد قالب الفيديو بشكل صحيح.", chat_id=chat_id, message_id=status_msg.message_id)
                    return
                    
                bot.edit_message_text("🧠 <b>جاري دمج وجهك مع مقطع الفيديو بالذكاء الاصطناعي (قد يستغرق ذلك دقيقة)...</b> ⏳", chat_id=chat_id, message_id=status_msg.message_id)
                
                model_ver = "9a42301c4e130b50e0e997bc32cbda5de547ecae20f262deec2bc837e408ec20"
                input_data = {
                    "swap_image": face_url,
                    "target_video": tpl["url"]
                }
                
                success, output = call_replicate_model(model_ver, input_data)
                
                if success:
                    bot.edit_message_text("🎬 <b>اكتمل الدمج! جاري إرسال الفيديو النهائي لك...</b> 🍿", chat_id=chat_id, message_id=status_msg.message_id)
                    bot.send_video(chat_id, output, caption="✨ <b>تم تبديل الوجوه بنجاح بالذكاء الاصطناعي!</b> 🎭🎬", reply_markup=get_main_menu_markup())
                else:
                    bot.edit_message_text(f"❌ فشلت معالجة الدمج: {output}", chat_id=chat_id, message_id=status_msg.message_id)
        ...
        elif state == "waiting_voice_audio" and message.content_type in ["audio", "voice"]:
            if not check_rate_limit_and_concurrency(chat_id, is_heavy=True):
                return
                
            active_tasks[chat_id] = True
            status_msg = bot.send_message(chat_id, "⏳ <b>جاري تحميل مقطعك الصوتي للمعالجة...</b>", parse_mode="HTML")
            try:
                fid = message.audio.file_id if message.content_type == "audio" else message.voice.file_id
                file_info = bot.get_file(fid)
                downloaded = bot.download_file(file_info.file_path)
                
                ext = "mp3" if message.content_type == "audio" else "ogg"
                local_path = os.path.join(MEDIA_DIR, f"voice_{chat_id}_{uuid.uuid4().hex[:6]}.{ext}")
                with open(local_path, "wb") as f:
                    f.write(downloaded)
                    
                bot.edit_message_text("🚀 <b>جاري رفع مقطعك الصوتي للخوادم السحابية...</b>", chat_id=chat_id, message_id=status_msg.message_id)
                
                audio_url = upload_to_tmpfiles(local_path)
                if not audio_url:
                    bot.edit_message_text("❌ فشل رفع الملف الصوتي، يرجى المحاولة مرة أخرى.", chat_id=chat_id, message_id=status_msg.message_id)
                    return
                    
                swap_info = user_swap_data.get(chat_id, {})
                singer_key = swap_info.get("selected_template_id")
                bot.edit_message_text(f"🧠 <b>جاري إعادة غناء مقطعك بنبرة صوت الفنان المختار عبر نموذج RVC...</b> ⏳", chat_id=chat_id, message_id=status_msg.message_id)
                
                model_ver = "6348c4cf3d4dc69772c1c9c4501a6b0c2669e2c65e8a78622c2bc837e408ec20"
                input_data = {
                    "audio": audio_url,
                    "model_name": singer_key
                }
                
                success, output = call_replicate_model(model_ver, input_data)
                
                if success:
                    bot.edit_message_text("🎵 <b>اكتمل التحويل! جاري إرسال المقطع الملحن الجديد...</b> 🎧", chat_id=chat_id, message_id=status_msg.message_id)
                    bot.send_audio(chat_id, output, caption=f"✨ <b>تم تحويل نبرة الصوت بنجاح!</b> 🎤🎼", reply_markup=get_main_menu_markup())
                else:
                    bot.edit_message_text(f"❌ فشلت معالجة الصوت: {output}", chat_id=chat_id, message_id=status_msg.message_id)
